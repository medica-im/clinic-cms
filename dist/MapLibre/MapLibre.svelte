...
import { Control, ControlGroup, ControlButton, DefaultMarker, Marker, Popup } from 'svelte-maplibre';
import * as openStreetMap from './style/openStreetMap.json';
let { data: points } = $props();
...
function getCenter() {
    ...
    const lngLat = latLng.slice().reverse();
    return lngLat;
}
</script>
{#if browser}
